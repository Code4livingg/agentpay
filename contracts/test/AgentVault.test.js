...
describe("AgentPay Smart Contracts", function () {
  let mockUSDC;
  let policyRegistry;
  let agentVault;
  let owner;
  ...
  // Deploy a simple MockERC20 token
  async function deployMockERC20() {
    const MockERC20 = await ethers.getContractFactory("MockERC20");
    const mockERC20 = await MockERC20.deploy("Mock USDC", "USDC", 6);
  ...
  describe("AgentVault - deposits", function () {
  ...
  describe("AgentVault - executePayment", function () {
    beforeEach(async function () {
      // Register agent
      const maxPerTx = ethers.parseUnits("100", 6);
      const dailyCap = ethers.parseUnits("500", 6);
    ...
    it("should execute payment within policy limits", async function () {
      ...
      await expect(
        ...
        .withArgs(agentId, recipient.address, paymentAmount, await ethers.provider.getBlock("latest").then(b => b.timestamp + 1));
  ...
  describe("AgentVault - daily reset", function () {
    beforeEach(async function () {
      // Register agent
      const maxPerTx = ethers.parseUnits("100", 6);
      const dailyCap = ethers.parseUnits("500", 6);
